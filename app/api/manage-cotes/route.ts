...
export async function POST(req: NextRequest) {
  ...
  if (action === 'upsert') {
    const { id, restaurant_id, nom, pourcentage, actif } = payload
    if (id) {
      const { error } = await admin.from('cotes').update({ nom, pourcentage, actif }).eq('id', id)
      if (error) return err(error.message, 500)
    } else {
      const { error } = await admin.from('cotes').insert({ restaurant_id, nom, pourcentage, actif })
      if (error) return err(error.message, 500)
    }
  ...
  if (action === 'archive') {
    const { id } = payload
    const { error } = await admin.from('cotes').update({ actif: false }).eq('id', id)
    if (error) return err(error.message, 500)
    return NextResponse.json({ ok: true })
  }
